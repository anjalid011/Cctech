...
void BezierWidget::initializeGL() {
    ...
    glPointSize(8.0f);
    glLineWidth(1.0f); // Make lines thinner
}
...
void BezierWidget::paintGL() {
    ...
    // Draws the 3D solid of revolution using quads
    glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
    glColor3f(0.6f, 0.6f, 0.6f);
    for (int i = 0; i < revolutionMesh.size() - 1; ++i) {
        for (int j = 0; j < revolutionMesh[i].size() - 1; ++j) {
            QVector3D normal = computeNormal(
                revolutionMesh[i][j],
                revolutionMesh[i + 1][j],
                revolutionMesh[i][j + 1]
            );
...
QVector3D BezierWidget::computeNormal(const QVector3D &p1, const QVector3D &p2, const QVector3D &p3) {
    QVector3D u = p2 - p1;
    QVector3D v = p3 - p1;
...
void BezierWidget::clear() {
    controlPoints.clear();  // Clear the control points
    curvePoints.clear();    // Clear the Bézier curve points
